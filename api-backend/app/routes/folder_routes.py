...
def create_Folder(new_Folder: folder_schema.FolderCreate, db: Session = Depends(get_db)):
    if db.query(Folder).filter(Folder.name == new_Folder.name).first():
        raise HTTPException(status_code=400, detail="Folder already exists!")
    
    try:
        db_new_Folder = Folder(name=new_Folder.name)
